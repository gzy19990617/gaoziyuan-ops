...
//v6: 将A存到shared_memory做一次转置，这样从a_shm取数据也可以用float4
...
__global__ void matrixMulCUDA(float* C,  float* A,  float* B, int M, int N, int K) {
    ...
    for (int s = 0; s < K; s += K_NUM_PER_BLOCK) {
        ...
        for (int i = 0; i < M_NUM_PER_THREAD; i++) {
            ...
            // 先写到寄存器里面
            FETCH_FLOA4(a_load_reg[0]) = FETCH_FLOA4(A[(y + i) * K + tx * N_NUM_PER_THREAD + s]);
            // 整个矩阵转置，同时块内行变为竖
            // 寄存器写到shm
        ...
        for (int k = 0; k < K_NUM_PER_BLOCK; k++) {
            ...
            // 这里的方向也要注意
            FETCH_FLOA4(a_reg[0]) = FETCH_FLOA4(a_shared[k][ty * N_NUM_PER_THREAD]);
